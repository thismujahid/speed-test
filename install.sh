...
# التأكد لو الأداة متثبتة قبل كده ويخرج فوراً (إلا لو المستخدم باعت علم الحذف لمنع التعارض)
...
# 2. التأكد من وجود speedtest وتثبيته بالطريقة الرسمية
if ! command -v speedtest &> /dev/null; then
    echo -e "\e[33m[+] Installing dependency: Speedtest CLI...\e[0m"
    sudo apt-get update -y && sudo apt-get install curl gnupg apt-transport-https -y &> /dev/null
    curl -fsSL https://packagecloud.io/ookla/speedtest-cli/gpgkey | sudo gpg --dearmor -o /etc/apt/keyrings/ookla_speedtest-cli-archive-keyring.gpg &> /dev/null
    echo "deb [signed-by=/etc/apt/keyrings/ookla_speedtest-cli-archive-keyring.gpg] https://packagecloud.io/ookla/speedtest-cli/ubuntu/ $(lsb_release -cs) main" | sudo tee /etc/apt/sources.list.d/speedtest.list &> /dev/null
...
# التحقق من خيار الحذف (-r أو --remove)
if [ "$1" == "-r" ] || [ "$1" == "--remove" ]; then
    echo -e "\e[33m[*] Removing tnet from your system...\e[0m"
    # مسح الملف التشغيلي نفسه
    sudo rm -f "$0"
    if [ $? -eq 0 ]; then
...
# تشغيل الفحص العادي لو مفيش flags
